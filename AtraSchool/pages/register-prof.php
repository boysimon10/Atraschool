...
{ 
    ...
    if(!empty($_POST['prenom']) AND !empty($_POST['nom']) AND !empty($_POST['mail']) AND !empty($_POST['mdp']) AND !empty($_POST['mdp2']) AND !empty($_POST['sexe']) AND !empty($_POST['jour']) AND !empty($_POST['mois']) AND !empty($_POST['année']) AND !empty($_POST['ecole']) AND !empty($_POST['matierensegn']) AND !empty($_POST['region']))
    {  
      ...
      if($prenomlength <=255)
      {
            ...
            if($nomlength <=255)
            {
                if($mdp == $mdp2){
                  ...
                  $insertmbr =$bdd->prepare("INSERT INTO users(prenom,nom,mail,mdp,sexe,jour,mois,annee,ecole,matierensegn,region,fonction,avatar)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
                  $insertmbr->execute(array($prenom,$nom,$mail,$mdp,$sexe,$jour,$mois,$annee,$ecole,$matierensegn,$region,$fonction,$avatar));
...
?>
   <span class="backtohome"><a href="index.php?page=home"><button><i class="i-left"></i>Retour à l'accueil</button></a></span>
    <div id="boxregister">
        ...
        <img width="200px"  src="images/mr.png"/>
        <span>
                <p>
    ...
    Eligendi, ipsa!
        ...
        <tr>
            <td>
             <label>Matiere ensegnée: </label>      
            </td>
            <td> 
             <select name="matierensegn">
